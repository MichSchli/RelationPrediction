...
class BasisGcnTimesDiag(MessageGcn):
    ...
    def compute_messages(self, sender_features, receiver_features):
        ...
        forward_messages = tf.reduce_sum(input_tensor=sender_terms * forward_type_scaling, axis=1)
        backward_messages = tf.reduce_sum(input_tensor=receiver_terms * backward_type_scaling, axis=1)

        return forward_messages, backward_messages

    def compute_coefficients(self):
        message_types = self.get_graph().get_type_indices()
        forward_type_scaling = tf.nn.sigmoid(tf.nn.embedding_lookup(params=self.C_forward, ids=message_types))
        backward_type_scaling = tf.nn.sigmoid(tf.nn.embedding_lookup(params=self.C_backward, ids=message_types))
