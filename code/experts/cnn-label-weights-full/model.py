...
class Model():
    ...
    def preprocess(self, triplets):
        ...
        relations = triplets[1] + 1

        sender_indices = np.hstack((triplets[0], triplets[2], np.arange(self.n_entities))).astype(np.int32)
        receiver_indices = np.hstack((triplets[2], triplets[0], np.arange(self.n_entities))).astype(np.int32)
        message_types = np.hstack((triplets[1]+1, triplets[1]+1+self.n_relations, np.zeros(self.n_entities, dtype=np.int32))).astype(np.int32)
        ...
        #Create TF sender-to-message list:
    ...
    def get_optimizer_parameters(self):
        return [('Minibatches', {'batch_size':self.batch_size, 'contiguous_sampling':False}),
                ...
                ('Adam', {'learning_rate':0.00005, 'historical_moment_weight':0.9, 'historical_gradient_weight':0.999}),
    ...
    def initialize_variables(self):
        ...
        type_initial = np.random.randn(self.n_relations*2+1, self.embedding_width, self.embedding_width).astype(np.float32)

        convolution_initials = [np.random.randn(self.embedding_width, self.embedding_width).astype(np.float32)
    ...
    def get_optimizer_loss(self):
        ...
        for W_layer in self.W_convolutions:
            ...
            regularization += tf.reduce_mean(tf.square(W_layer))
    ...
    def get_optimizer_weights(self):
        return [self.W_embedding, self.W_relation, self.W_type] + self.W_convolutions
    ...
    def save(self, filename, variables):
        store_package = variables + [self.n_entities, self.n_relations]
    ...
    def load(self, filename):
        ...
        self.W_convolutions = [tf.Variable(store_package[x]) for x in range(3,-2)]
