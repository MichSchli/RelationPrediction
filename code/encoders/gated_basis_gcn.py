...
class GatedBasisGcn(Model):
    ...
    def local_initialize_train(self):
        ...
        glorot_var_combined = np.sqrt(3/(vertex_matrix_shape[0]*2 + edge_matrix_shape[0] + vertex_matrix_shape[1]))

        vertex_projection_sender_initial = np.random.normal(0, glorot_var_combined, size=vertex_matrix_shape).astype(np.float32)
        vertex_projection_receiver_initial = np.random.normal(0, glorot_var_combined, size=vertex_matrix_shape).astype(np.float32)
        edge_projection_initial = np.random.normal(0, glorot_var_combined, size=edge_matrix_shape).astype(np.float32)

        vertex_gate_sender_initial = np.random.normal(0, glorot_var_combined, size=vertex_matrix_shape).astype(np.float32)
        vertex_gate_receiver_initial = np.random.normal(0, glorot_var_combined, size=vertex_matrix_shape).astype(np.float32)
        edge_gate_initial = np.random.normal(0, glorot_var_combined, size=edge_matrix_shape).astype(np.float32)

        gate_transform_matrix = np.random.normal(0, np.sqrt(1/(2*self.embedding_width)), size=(self.embedding_width, self.embedding_width)).astype(np.float32)
        ...
        gate_post_bias = np.zeros(self.embedding_width).astype(np.float32)

        self.V_proj_sender = tf.Variable(vertex_projection_sender_initial)
        self.V_proj_receiver = tf.Variable(vertex_projection_receiver_initial)
        self.E_proj = tf.Variable(edge_projection_initial)

        self.V_gate_sender = tf.Variable(vertex_gate_sender_initial)
        self.V_gate_receiver = tf.Variable(vertex_gate_receiver_initial)
        self.E_gate = tf.Variable(edge_gate_initial)

        self.W_gate_transform = tf.Variable(gate_transform_matrix)

        self.B_message = tf.Variable(message_bias)
        self.B_gate_pre = tf.Variable(gate_pre_bias)
        self.B_gate_post = tf.Variable(gate_post_bias)

    def local_get_weights(self):
        return [self.V_proj_sender, self.V_proj_receiver, self.E_proj,
                self.V_gate_sender, self.V_gate_receiver, self.E_gate,
                self.W_gate_transform,
                self.B_message, self.B_gate_pre, self.B_gate_post]
    ...
    def get_vertex_features(self, mode='train'):
        ...
        if self.onehot_input:
            return sender_index_vector, receiver_index_vector
        else:
            ...
            receiver_codes = tf.nn.embedding_lookup(code, receiver_index_vector)

            return sender_codes, receiver_codes
    ...
    def compute_vertex_embeddings(self, mode='train'):
        if self.vertex_embedding_function[mode] is None:
            sender_features, receiver_features = self.get_vertex_features(mode=mode)
            type_index_vector = self.graph_representation.get_type_indices()
            messages = self.compute_messages(receiver_features, sender_features, type_index_vector)
            gates = self.compute_gate_values(receiver_features, sender_features, type_index_vector)
    ...
    def compute_gate_values(self, receiver_features, sender_features, type_index_vector):
        sender_gate_terms = self.dot_or_lookup(sender_features, self.V_gate_sender)
        receiver_gate_terms = self.dot_or_lookup(receiver_features, self.V_gate_receiver)
        type_terms = tf.nn.embedding_lookup(self.E_gate, type_index_vector)
        gate_energies = tf.matmul(tf.nn.relu(sender_gate_terms + receiver_gate_terms + type_terms + self.B_gate_pre),
                                  self.W_gate_transform) + self.B_gate_post
        flat_energies = tf.reshape(gate_energies, [-1])
        gates = self.apply_edge_softmax(flat_energies)
        return gates

    def compute_messages(self, receiver_features, sender_features, type_index_vector):
        sender_terms = self.dot_or_lookup(sender_features, self.V_proj_sender)
        receiver_terms = self.dot_or_lookup(receiver_features, self.V_proj_receiver)
        type_terms = tf.nn.embedding_lookup(self.E_proj, type_index_vector)
        messages = tf.nn.relu(sender_terms + receiver_terms + type_terms + self.B_message)
        return messages
